              ...
              <div class="container">
                ...
                <div class="collapse navbar-collapse" id="collapsibleNavId">
                    ...
                    <div class="d-flex my-2 my-lg-0">
                        <!-- <input class="form-control me-sm-2" list="datalistOptions" id="searchProduct" placeholder="Search"> -->
                        <input class="form-control" list="datalistOptions1" id="searchProduct1" placeholder="Type to search...">
                        <datalist id="datalistOptions1">
                            <?php
                            $conn = mysqli_connect("localhost", "root", "", "db_web_da");
                            $sql1 = "SELECT noidung  FROM `dangtin` WHERE noidung  LIKE '%_%' ORDER BY time DESC LIMIT 5 ;  ";
                            $ketqua1 = mysqli_query($conn, $sql1);
                            // $row = mysqli_fetch_array($ketqua1);
                            while ($row = mysqli_fetch_array($ketqua1)) {
                                ?>
                                <option value='<?php echo  "".$row['noidung']."";?>' data-url='<?php echo "./chitietcongdong.php?noidung=". $row['noidung']; ?>'>
                                <?php
                            }
                            ?>
                        </datalist>
                        <script>
                            document.getElementById("searchProduct1").addEventListener("input", function() {
                            var inputValue = this.value.toLowerCase();
                            var dataListOptions = document.getElementById("datalistOptions1").options;
